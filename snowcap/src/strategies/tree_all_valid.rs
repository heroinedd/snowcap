...
use log::{debug, error, info, trace, LevelFilter, STATIC_MAX_LEVEL};
use std::marker::PhantomData;
use std::time::{Duration, SystemTime};

/// # The Tree Strategy that can find all valid orderings
///
...
pub struct TreeAllValidStrategy<O>
where
...
impl<O> Strategy for TreeAllValidStrategy<O>
where
    O: ModifierOrdering<ConfigModifier>,
{
    fn new(
        ...
        loop {
            ...
            if let Some(s) = stack.last_mut() {
                ...
                if s.rem_mod.is_empty() {
                    ...
                    // println!("find {:?} valid orders", all_valid_ordering.len());
                    pop_stack = true;
                }
